  ...
  async canActivate(): Promise<boolean> {
    return this.authService.isAuthenticatedPromise().then(async (isAuthenticated) => {
      if (isAuthenticated) {
        ...
        this.router.navigate(['/trabajador/login']); // Redirige al dashboard si está autenticado
...
canActivate(): boolean {
  if (this.authService.isAuthenticated()) {
    ...
    this.router.navigate(['/trabajador/login']); // Redirige al login si no está autenticado
    return false;
  }
} */
