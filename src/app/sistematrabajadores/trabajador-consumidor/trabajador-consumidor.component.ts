...
@Component({
    ...
    standalone: true,
    imports: [RouterOutlet, ScrollPanelModule, DropdownModule, ButtonModule, CommonModule, ImageModule, LoadingComponent],
...
export class TrabajadorConsumidorComponent {
    ...
    navigateToPage2(){
        this.router.navigate(['/trabajador/dashboard/profile']);
    }
    ...
    async getData() {
        ...
        if (!result.isReviewed){
            this.router.navigate(['/trabajador/confirmacion-postulacion']);
        }
    }
}
