...
export class OnboardingComponent implements AfterViewChecked {
  ...
  navigateToPage4() {
    this.router.navigate(['/trabajador/dashboard/validacion-trabajador']); // Reemplaza '/nueva-ruta' por la ruta a la que quieras redirigir
  }
  ...
  ngAfterViewChecked(): void {
    ...
    if (this.currentStep === 5 && !this.mapInitialized) {
      this.initializeMap();
  ...
  constructor(private cdr: ChangeDetectorRef, private config: PrimeNGConfig, private messageService: MessageService, private fb: FormBuilder, private router: Router, private authService: AuthServiceTrabajadores,) {
    this.onboardingForm = this.fb.group({
      fullname: ['', Validators.required],
      dni: ['', Validators.required],
      birthday: ['', Validators.required],
      selectedProfessions: [null, Validators.required],
  ...
  ngOnInit() {
    this.professions = [
      { name: 'Albañil', code: 'ALB' },
      { name: 'Electricista', code: 'ELE' },
      { name: 'Plomero', code: 'PLO' },
      { name: 'Carpintero', code: 'CAR' },
      { name: 'Cocinero', code: 'COC' },
      { name: 'Pintor', code: 'PIN' },
      { name: 'Jardinero', code: 'JAR' },
      { name: 'Mecánico', code: 'MEC' },
      { name: 'Estilista', code: 'EST' },
      { name: 'Taxista', code: 'TAX' }
    ];
  ...
  isStepValid(step: number): boolean {
    switch (step) {
      ...
      case 4:
        return (this.onboardingForm.get('selectedProfessions')?.valid ?? false) &&
          (this.onboardingForm.get('valueTextArea')?.valid ?? false);
      case 5:
  ...
  async submit() {
    ...
    try {
      ...
      await this.authService.updateDocument('workers', this.user.uid, datosActualizados);
      this.onboardingForm.reset();
      this.router.navigate(['/trabajador/dashboard/jobs']);
